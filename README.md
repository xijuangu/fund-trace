...
- 腾讯财经批量股票行情，股票列表一次请求
- 支持基金历史净值和股票日 K 线查询，默认 30 天，可自定义天数

**Sparkline 趋势线**
- 0% 锚定绝对坐标系：▁▂▃▄▅▆▇█，0% 固定在 ▄，正数往上、负数往下
- 数据源为日涨跌幅，每个块直接反映当天涨跌幅度
- 逐块独立染色：正值绿色、负值红色、▄（0% 附近）灰色
- 最右块自动追加当日实时涨跌幅，始终与 Change% 列同步
- TUI 刷新以数据库资产列表为准，配置文件作为空库兜底；因此通过 TUI/CLI 添加的股票都会参与行情和 Trend 刷新

**持久化**
- SQLite WAL 模式存储，崩溃安全
- 六张表：`funds`、`assets`、`nav_snapshots`、`price_snapshots`、`alerts`、`daily_summary`
- 首次运行自动建库建表
...
基金历史净值 + 技术分析。配置中明确为股票的代码会自动显示股票历史。

```bash
$ fund-trace history 011513 --days 60
```

### `fund-trace history stock <code> [--days N]`

A 股日 K 线收盘价 + 技术分析。

```bash
$ fund-trace history stock 600519 --days 60
$ fund-trace history stock sh 600519 --days 60
```
...
```bash
$ fund-trace alert set stock 600519 --drop 3
$ fund-trace alert set stock sh 600519 --rise 5
```
...
| 东方财富 `push2his.eastmoney.com` | A 股日 K 线 |

> ⚠️ 腾讯财经和东方财富行情接口为公开非正式接口，仅用于个人行情查看，不保证长期稳定，不构成投资建议。
...
└── internal/
    ...
    ├── store/       # SQLite WAL，assets/funds/price_snapshots 表 CRUD
